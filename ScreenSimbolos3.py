"""
Esta clase contiene la ventana de resultados para Señales No Convencionales - 3 Símbolos.
"""
...
class ScreenSimbolos3(QMainWindow):
    def __init__(self, widget,frameSizeWidth, frameSizeHeight, fSample, nTotalSimb, sizeHeight):
        ...
        """
        super(ScreenSimbolos3, self).__init__()
        loadUi("UIs\SenalesNC_Simb_3S.ui", self)
        self.widget = widget
        ...
        # Horizontal sliders par la frecuencia de portadora
    ...
    def chequearSimbsIguales(self,s1,s2,s3):
        """
    ...
    def graficarSimb(self, s1, s2, s3, tSimb, t):
        """
        ...
        yMax = np.amax(np.array([np.amax(abs(s1)), np.amax(abs(s2)), np.amax(abs(s3))]))
        ...
        self.axs1 = self.figure_simbGraph.add_subplot(311)
        ...
        self.axs2 = self.figure_simbGraph.add_subplot(312)
        ...
        self.axs3 = self.figure_simbGraph.add_subplot(313)
    ...
    def graficarConstAntes(self, s1, s2, s3, U1, U2, tSimb):
        """
        ...
        ejeX = np.array([x1, x2, x3])
        ejeY = np.array([y1, y2, y3])
    ...
    def graficarConstDespues(self, senal, U1, U2, tSimb, coord, nMuestrasSimb):
        ...
        coordX = np.array([x1, x2, x3])
        coordY = np.array([y1, y2, y3])

        ejeX = np.concatenate((detU1, coordX))
        ejeY = np.concatenate((detU2, coordY))

        size = np.concatenate((np.full((1, detU1.size), 5), np.full((1, 3), 300)), axis=1)
        z = np.concatenate((np.full((1, detU1.size), 0.5), np.full((1, 3), 0)), axis=1)
    ...
    def simular(self):
        ...
        try:
            ...
            simbsIguales = self.chequearSimbsIguales(s1,s2,s3)
            ...
            simbolos_senal = (s1, s2, s3, 0)
            tipoOndas = (tipoOnda1, tipoOnda2, tipoOnda3, 0)
            frecuencias = (frecuencia1, frecuencia2, frecuencia3, 0)

            U1, U2 = gramSchmidt(s1, s2, s3, 0, nMuestrasSimb, self.fSample)
            senal, t2, bits = senalAleatoria(s1, s2, s3, 0, tSimb, nMuestrasSimb, nroSimb, self.nTotalSimb)

            self.graficarSimb(s1, s2, s3, tSimb, t)
            self.graficarBases(U1, U2, tSimb, t)

            coord,dosBases = self.graficarConstAntes(s1, s2, s3, U1, U2, tSimb)
            ...
            senalDet, rec = deteccionDistancia(coord, s1, s2, s3, 0, nroSimb, detU1, detU2, self.nTotalSimb, nMuestrasSimb)

            pe, primerError, cont = calcularPe(self.nTotalSimb, rec, bits)

            senalBBPB = senalBandabasePasabanda(s1, s2, s3, 0, nroSimb, self.fSample, nMuestrasSimb)
            ...
            o_s1s2, o_s1s3, o_s2s3 = ortogonalidad
            p_s1s2, p_s1s3, p_s2s3 = polaridad
            pp_s1s2, pp_s1s3, pp_s2s3 = proporcionalidad

            E1, E2, E3 = energia
            numCiclosEnterosS1, numCiclosEnterosS2, numCiclosEnterosS3 = numCiclos
