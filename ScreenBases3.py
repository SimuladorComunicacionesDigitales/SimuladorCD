"""
Esta clase contiene la ventana de resultados para Señales No Convencionales - Bases - 3 Símbolos.
"""
...
class ScreenBases3(QMainWindow):
    def __init__(self, widget,frameSizeWidth, frameSizeHeight, fSample, nTotalSimb, sizeHeight):
        ...
        """
        super(ScreenBases3, self).__init__()
        loadUi("UIs\SenalesNC_Bases_3S.ui", self)
        self.widget = widget
    ...
    def graficarSimb(self, s1, s2, s3, tSimb, t):
        ...
        self.axs1.annotate(f'Amplitud: {format(maX, "<10.4f")}', (1, 1), xycoords='axes pixels',weight='bold',fontsize=self.sizeHeight*0.01)
        ...
        self.axs2.annotate(f'Amplitud: {format(maX, "<10.4f")}', (1, 1), xycoords='axes pixels',weight='bold',fontsize=self.sizeHeight*0.01)
        ...
        self.axs3.grid()
        maX = np.amax(abs(s3Graf))
        self.axs3.annotate(f'Amplitud: {format(maX, "<10.4f")}', (1, 1), xycoords='axes pixels',weight='bold',fontsize=self.sizeHeight*0.01)
    ...
    def simular(self):
        ...
        try:
            ...
            #datosS1 = (tipoOnda_Opcion1, amplitud1, frecuencia1, fase1, t1s1, t2s1)
            #datosS2 = (tipoOnda_Opcion2, amplitud2, frecuencia2, fase2, t1s2, t2s2)
            ...
            xS3 = float(self.XS3.text())
            yS3 = float(self.YS3.text())
            ...
            coord = self.graficarConstAntes(s1, s2,s3, U1, U2, tSimb)
            ...
            if math.isclose(factorLambda, 0, abs_tol=0.01):
                ...
                # Coordenadas S3
                if xS3.is_integer():
                    self.leXS3.setText(str(int(xS3)))
                else:
                    self.leXS3.setText(format(xS3, "10.3f"))

                if yS3.is_integer():
                    self.leYS3.setText(str(int(yS3)))
                else:
                    self.leYS3.setText(format(yS3, "10.3f"))
            else:
